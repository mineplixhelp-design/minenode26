// Struktur Sidebar Menu menggunakan URL File Asli
const sidebarMenu = [
  { id: "home.html", label: "Home", type: "single" },
  {
    id: "gameplay",
    label: "Panduan Gameplay",
    type: "dropdown",
    children: [
      { id: "tutorial-land.html", label: "Tutorial Land" },
      { id: "tutorial-economy.html", label: "Tutorial Economy" },
      { id: "tutorial-essentials.html", label: "Tutorial Home Teleport Rtp Warp Essentials" }
    ]
  },
  { id: "rules.html", label: "Rules & Regulasi", type: "single" },
  { id: "economies-stats.html", label: "Economies and Stats Update", type: "single" },
  { id: "ranks.html", label: "Server Ranks Badge", type: "single" }
];
...
function initSidebarNav() {
  ...
  sidebarMenu.forEach(menu => {
    if (menu.type === "single") {
      ...
      itemA.className = `nav-item ${menu.id === currentFile ? 'active' : ''}`;
      itemA.href = menu.id;
      itemA.innerHTML = `<span class="label">${menu.label}</span>`;
      navContainer.appendChild(itemA);
    } else if (menu.type === "dropdown") {
      const hasActiveChild = menu.children.some(child => child.id === currentFile);
      ...
      menu.children.forEach(child => {
        ...
        childA.className = `nav-item ${child.id === currentFile ? 'active' : ''}`;
...
async function loadPageData(jsonTarget) {
  try {
    const response = await fetch(`./pages/${jsonTarget}.json`);
    ...
    document.getElementById('contentBody').innerHTML = `<div class="callout-box callout-info">Gagal memuat konten dari pages/${jsonTarget}.json</div>`;
  }
}
